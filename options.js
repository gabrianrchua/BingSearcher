const applyBtn = document.getElementById("applyTermsBtn");
...
function renderTermBoxes() {
	...
	for (let i in searchTerms) {
		finalHtml += "<input type='text' id='term" + i + "' value='" + searchTerms[i] + "'><br>";
	}
	termsDiv.innerHTML = finalHtml;
	for (let i in searchTerms) {
		document.getElementById("term" + i).addEventListener("focusout", function() {
			updateTerm(i);
		});
	}
}
